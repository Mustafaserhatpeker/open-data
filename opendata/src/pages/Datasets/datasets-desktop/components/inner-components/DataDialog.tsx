...
export default function DataDialog({ dataset }: { dataset: DatasetItem }) {
  const meta = getTypeMeta(dataset.formats?.[0]?.formatName || "");
    ...
    <Dialog>
      ...
      >
        ...
        <div className="grid grid-cols-1 gap-4 text-sm md:grid-cols-2">
          ...
          <InfoRow
            label="ID"
            value={dataset ? String(dataset._id) : "-"}
            icon={<Hash className="h-4 w-4 opacity-80" />}
          />
          <InfoRow
            label="Kategori"
            value={dataset?.categories?.[0]?.categoryName || "-"}
          ...
          <InfoRow
            label="Kurum"
            value={dataset?.organization.organizationName || "-"}
          ...
          <div className="flex flex-wrap gap-1.5">
            {dataset?.tags && dataset.tags.length > 0 ? (
              dataset.tags.map((tag) => (
                <Badge key={tag._id} variant="outline" className="px-2 py-0.5">
                  {tag.tagName}
                </Badge>
              ))
            ) : (
