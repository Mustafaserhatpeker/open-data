...
import { getDatasets, getMyDatasets } from "@/services/dataset.service";
...
export function useDatasets() {
  ...
  const { data, isLoading, isError, error, refetch, isFetching } = useQuery({
    ...
    enabled: !showMyDatasets, // sadece genel veri için
  });
  ...
  const {
    ...
    queryKey: ["my-datasets", params, accessToken],
    queryFn: () => getMyDatasets(params, accessToken),
    enabled: !!accessToken && showMyDatasets, // sadece toggle aktifken
  });

  // ---- response mapping
  const activeData = showMyDatasets ? myData : data;

  const items: DatasetItem[] = activeData?.data?.data ?? [];
  ...
  return {
    ...
    // react-query state
    isLoading: showMyDatasets ? isMyLoading : isLoading,
    isFetching: showMyDatasets ? isMyFetching : isFetching,
    isError: showMyDatasets ? isMyError : isError,
    error: showMyDatasets ? myError : error,
    refetch: showMyDatasets ? refetchMy : refetch,
