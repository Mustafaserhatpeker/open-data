...
export type DatasetOrgFormValues = z.infer<typeof FormSchema>;

interface AddDataOrgDialogProps {
    organizationId: string;
    organizationName: string;
}

export function AddDataOrgDialog({ organizationId, organizationName }: AddDataOrgDialogProps) {
    ...
    const createMutation = useMutation({
        mutationFn: async (payload: DatasetOrgFormValues) => {
    ...
    const onSubmit = (values: DatasetOrgFormValues) => createMutation.mutate(values);
        ...
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="sm:max-w-[640px]">
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                    <DialogHeader>
                        <DialogTitle>{organizationName} – Veri Seti Ekle</DialogTitle>
                        <DialogDescription>
                            Bu organizasyona ait yeni bir veri seti oluşturun.
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    <div className="grid gap-4">
                        ...
                        <div className="grid gap-2">
                            ...
                            <Controller
                                name="description"
                                control={control}
                                render={({ field }) => <Textarea id="description" placeholder="Veri seti açıklaması" {...field} />}
                            />
                            ...
                            <Controller
                                ...
                                render={({ field }) => (
                                    <MultiSelect values={field.value} onValuesChange={field.onChange}>
                                        ...
                                        <MultiSelectContent search={{ placeholder: "Ara...", emptyMessage: "Kategori yok" }}>
                                            <MultiSelectGroup>
                            ...
                            <Controller
                                ...
                                render={({ field }) => (
                                    <MultiSelect values={field.value} onValuesChange={field.onChange}>
                                        ...
                                        <MultiSelectContent search={{ placeholder: "Ara...", emptyMessage: "Etiket yok" }}>
                                            <MultiSelectGroup>
                            ...
                            <Controller
                                ...
                                render={({ field }) => (
                                    <MultiSelect values={field.value} onValuesChange={field.onChange}>
                                        ...
                                        <MultiSelectContent search={{ placeholder: "Ara...", emptyMessage: "Format yok" }}>
                                            <MultiSelectGroup>
                        ...
                        <div className="grid gap-2">
                            ...
                            />
                            {errors.licenseID && <p className="text-sm text-red-500">{errors.licenseID.message}</p>}
                        </div>

                        <div className="flex items-center justify-between border rounded-lg p-3">
                            <div>
                                <Label htmlFor="isPublic">Herkese Açık</Label>
                                <p className="text-xs text-muted-foreground">Veri setini herkes görebilsin mi?</p>
                            </div>
                            <Controller
                                name="isPublic"
                                control={control}
                    ...
                    {createMutation.isError && (
                        <p className="text-sm text-red-500">{(createMutation.error as Error)?.message ?? "Hata oluştu"}</p>
                    )}
                </form>
            </DialogContent>
        </Dialog>
